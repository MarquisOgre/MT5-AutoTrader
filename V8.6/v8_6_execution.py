...
"""
DEXORZO Innovations
MT5 AutoTrader V8.6 - Dynamic M5
...
APP_TITLE = "MT5 AUTOTRADER V8.6 - DYNAMIC M5"
...
TRADES_FILE=REPORT/"v8_6_dynamic_m5_trades.csv"
STATE_FILE=RUNTIME/"v8_6_day_state.json"
STOP_FILE=Path("STOP_V8_6")
PAUSE_FILE=Path("PAUSE_V8_6")
RESUME_FILE=Path("RESUME_V8_6")
# Runtime state for partial-take-profit management.
PARTIAL_STATE_FILE=RUNTIME/"v8_6_partial_state.json"
...
def guard():
    ...
    if "MetaQuotes-Demo" not in str(a.server or ""):
        raise RuntimeError(f"SAFETY LOCK: V8.6 is restricted to MetaQuotes-Demo. Current server: {a.server}")
...
def discover_markets():
    ...
    markets=[]; counts={"FX":0,"CRYPTO":0,"METALS":0,"INDEX":0,"OTHER":0}
    candidates=[]
    for item in all_syms:
        name=str(item.name); cls=classify_symbol(name,item)
        counts[cls]=counts.get(cls,0)+1
        if cls in ("FX","CRYPTO","METALS","INDEX"):
            candidates.append((name,cls,item))

    # Disabled symbols are excluded before any expensive terminal calls.
    candidates=[x for x in candidates if int(getattr(x[2],"trade_mode",0))!=getattr(mt5,"SYMBOL_TRADE_MODE_DISABLED",0)]

    # Prefer 24x7 assets first on the weekend, but retain a reasonable FX sample.
    crypto=[x for x in candidates if x[1]=="CRYPTO"]
    metals=[x for x in candidates if x[1]=="METALS"]
    indices=[x for x in candidates if x[1]=="INDEX"]
    fx=[x for x in candidates if x[1]=="FX"]
    # Bound the probe set to prevent hundreds of slow symbol_select/tick calls.
    # If the broker exposes more, rotate the candidate tail on later scans.
    ordered=crypto[:60]+metals[:30]+indices[:30]+fx[:40]
    seen=set()
    for name,cls,item in ordered:
        if name not in seen:
            markets.append(name); seen.add(name)
    return markets,counts


def get_live_tick(symbol, max_age):
    """Return executable tick if fresh; also expose bar/tick diagnostics."""
    now=int(time.time())
    ...
    # Ask MT5 for recent tick history as a diagnostic/fallback. This does not
    # make an old quote executable; it only tells the dashboard whether MT5 has
    # historical tick data for the symbol.
    try:
        recent=mt5.copy_ticks_from(symbol, datetime.fromtimestamp(now-3600, tz=timezone.utc), 200, mt5.COPY_TICKS_ALL)
        if recent is not None and len(recent):
            last_ts=int(recent[-1]["time"])
            age=max(0,now-last_ts)
            if age<=max_age:
                return None,age,"HISTORY_RECENT_NO_QUOTE"
            return None,age,"HISTORY_STALE"
    except Exception:
        pass
...
def market_quality(symbol):
    try:
        ...
        if not mt5.symbol_select(symbol,True):return None,"SYMBOL_SELECT_FAILED"
        tick,age_s,tick_source=get_live_tick(symbol,MIN_TICK_AGE_SECONDS.get(cls,300))
        bar=bar_diagnostic(symbol)
        if tick is None:
            if tick_source=="HISTORY_RECENT_NO_QUOTE": return None,"RECENT_HISTORY_NO_EXECUTABLE_TICK"
            if tick_source=="HISTORY_STALE" and bar["bar"]=="RECENT": return None,"RECENT_M5_NO_EXECUTABLE_TICK"
            if bar["bar"]=="STALE": return None,"STALE_M5_NO_TICK"
...
def select_pairs():
    ...
    for idx,symbol in enumerate(discovered,1):
        if idx==1 or idx%10==0:
            print(f"\rScanning markets: {idx}/{total} ...",end="",flush=True)
        ...
        if q: good.append(q)
            ...
            if len(samples)<12 and classify_symbol(symbol,mt5.symbol_info(symbol)) in ("CRYPTO","METALS","INDEX"):
                samples.append((symbol,reason,bar_diagnostic(symbol)))
    print(f"\rScanning markets: {total}/{total} ... done.{' '*20}")
    groups={c:sorted([q for q in good if q["asset_class"]==c],key=lambda q:q["score"],reverse=True) for c in ("FX","CRYPTO","METALS","INDEX")}
    if groups["FX"]:
        ordered=groups["FX"]+groups["CRYPTO"]+groups["METALS"]+groups["INDEX"]
    else:
        ordered=groups["CRYPTO"]+groups["METALS"]+groups["INDEX"]+groups["FX"]
...
def dashboard(a,selected,states,last,error,reasons=None,scanned=0,inventory=None,samples=None):
    ...
    os.system("cls")
    print("+"+"-"*80+"+")
    print("|"+"DEXORZO  V8.6  |  INTELLIGENT 24x7 DYNAMIC M5".center(80)+"|")
    ...
    print("+"+"-"*80+"+")
    print("|"+"24x7 MARKET INVENTORY".center(80)+"|")
    inv=inventory or {}
    ...
    print("+"+"-"*80+"+")
    print("|"+"DYNAMIC MARKET SELECTION".center(80)+"|")
    if selected:
        for q in selected:
        ...
        print("|"+"No qualifying markets right now. Scanner will keep looking.".ljust(80)+"|")
    if reasons:
    ...
    try:
        while True:
            ...
            if ctl=="STOP":
                ...
                dashboard(a,[],states,last,"STOP_V8_6 active - halted",{},0,{},[])
                break
            manage_positions()
