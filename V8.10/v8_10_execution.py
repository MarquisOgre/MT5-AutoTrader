...
"""
DEXORZO Innovations
MT5 AutoTrader V8.10 - Resilient Live Feed M5
...
APP_TITLE = "MT5 AUTOTRADER V8.10 - RESILIENT LIVE FEED M5"
BRAND = "Created By DEXORZO Innovations"
MAGIC = 762017
SCAN_SECONDS = 10
SESSION_REFRESH_SECONDS = 60
FAST_SCAN_NO_HISTORY = True
LIVE_DISCOVERY_BATCH = 40
MAX_LIVE_TICK_AGE_SECONDS = {"FX":180,"CRYPTO":120,"METALS":300,"INDEX":300,"OTHER":300}
...
TRADES_FILE=REPORT/"v8_10_resilient_m5_trades.csv"
STATE_FILE=RUNTIME/"v8_10_day_state.json"
STOP_FILE=Path("STOP_V8_10")
PAUSE_FILE=Path("PAUSE_V8_10")
RESUME_FILE=Path("RESUME_V8_10")
# Runtime state for partial-take-profit management.
PARTIAL_STATE_FILE=RUNTIME/"v8_10_partial_state.json"
...
def connect():
    """Connect to MT5 without terminating the bot when the terminal is unavailable."""
    try:
        ...
        if a is None or t is None:
            err=f"MT5 account/terminal information unavailable: {mt5.last_error()}"
            try: mt5.shutdown()
        ...
        return None, f"MT5 connection exception: {e}"

def guard():
    """Return (account, status) instead of raising for temporary MT5 disconnects."""
    try:
        a=mt5.account_info(); t=mt5.terminal_info()
        if a is None or t is None:
            return None, "MT5 TERMINAL OFFLINE / ACCOUNT INFORMATION UNAVAILABLE"
        if "MetaQuotes-Demo" not in str(a.server or ""):
            return None, f"SAFETY LOCK: V8.10 is restricted to MetaQuotes-Demo. Current server: {a.server}"
        if not bool(getattr(a,"trade_allowed",False)):
            return None, "PAUSED: MT5 account trading is not allowed"
        if not bool(getattr(t,"trade_allowed",False)):
            return None, "PAUSED: MT5 Algo Trading is disabled"
        return a, ""
    except Exception as e:
        return None, f"MT5 connection unavailable: {e}"
...
def get_live_tick(symbol, max_age):
    ...
    if info is not None:
        bid=float(getattr(info,"bid",0) or 0); ask=float(getattr(info,"ask",0) or 0)
        tms=int(getattr(info,"time_msc",0) or 0); ts=int(getattr(info,"time",0) or 0)
        if tms<=0 and ts>0: tms=ts*1000
        if bid>0 and ask>0 and tms>0:
            age_ms=max(0,now_ms-tms)
            if age_ms<=max_age*1000: return (bid,ask,tms),age_ms/1000.0,"SYMBOL_INFO"
    try: tick=mt5.symbol_info_tick(symbol)
    except Exception: tick=None
    if tick is not None:
        bid=float(getattr(tick,"bid",0) or 0); ask=float(getattr(tick,"ask",0) or 0)
        tms=int(getattr(tick,"time_msc",0) or 0); ts=int(getattr(tick,"time",0) or 0)
        if tms<=0 and ts>0: tms=ts*1000
        if bid>0 and ask>0 and tms>0:
            age_ms=max(0,now_ms-tms)
            if age_ms<=max_age*1000: return (bid,ask,tms),age_ms/1000.0,"TICK"
            return None,age_ms/1000.0,"STALE_TICK"
...
def market_quality(symbol):
    try:
        ...
        quote,age_s,tick_source=get_live_tick(symbol,MAX_LIVE_TICK_AGE_SECONDS.get(cls,300))
        if quote is None:return None,"STALE_LIVE_QUOTE" if tick_source=="STALE_TICK" else "NO_LIVE_QUOTE"
        bid,ask,_=quote; mid=(ask+bid)/2
        ...
        return {"symbol":symbol,"asset_class":cls,"spread_pct":spread_pct,"atr":atr,"atr_points":atr_points,"activity":activity,"tick_age":age_s,"tick_source":tick_source,"bid":bid,"ask":ask,"score":score},"OK"
    except Exception as e:return None,"ERROR_"+type(e).__name__

def select_pairs():
    discovered,inventory=discover_markets(); good=[]; reasons={}; samples=[]
    total=len(discovered); live_by_class={c:0 for c in ("FX","CRYPTO","METALS","INDEX","OTHER")}
    for batch_start in range(0,total,LIVE_DISCOVERY_BATCH):
        batch=discovered[batch_start:batch_start+LIVE_DISCOVERY_BATCH]
        for symbol in batch:
            try: mt5.symbol_select(symbol,True)
            except Exception: pass
        time.sleep(0.25)
        for idx,symbol in enumerate(batch,batch_start+1):
            if idx==1 or idx%10==0 or idx==total: print(f"Live discovery {market_mode_label()}: {idx}/{total} ...",end="",flush=True)
            q,reason=market_quality(symbol); cls=classify_symbol(symbol,mt5.symbol_info(symbol))
            if q:
                good.append(q); live_by_class[cls]=live_by_class.get(cls,0)+1
            else:
                reasons[reason]=reasons.get(reason,0)+1
                if reason in ("NO_LIVE_QUOTE","STALE_LIVE_QUOTE") and len(samples)<12 and cls in ("CRYPTO","METALS","INDEX","FX"):
                    info=mt5.symbol_info(symbol)
                    samples.append((symbol,reason,{"bid":getattr(info,"bid",0) if info else 0,"ask":getattr(info,"ask",0) if info else 0,"time_msc":getattr(info,"time_msc",0) if info else 0}))
    print(f"Live discovery {market_mode_label()}: {total}/{total} ... done.{' '*20}")
    ...
    diagnostics={"live_by_class":live_by_class,"live_total":len(good),"discovered":total}
    return ordered[:MAX_SELECTED_PAIRS],discovered,reasons,inventory,samples,diagnostics
...
def modify_position(p,new_sl=None,new_tp=None):
    ...
    req={"action":mt5.TRADE_ACTION_SLTP,"symbol":p.symbol,"position":p.ticket,"sl":sl,"tp":tp,"magic":MAGIC,"comment":"DEXORZO-V8.10-MGMT"}
...
def send_entry(sig,a):
    ...
    req={"action":mt5.TRADE_ACTION_DEAL,"symbol":s,"volume":vol,"type":typ,"price":entry,
         "sl":sl,"tp":tp,"deviation":DEVIATION,"magic":MAGIC,"comment":"DEXORZO-V8.10",
...
def dashboard(a,selected,states,last,error,reasons=None,scanned=0,inventory=None,samples=None,diagnostics=None):
    ...
    if selected:
        ...
        print("|"+msg.ljust(80)+"|")
    d=diagnostics or {}
    lb=d.get("live_by_class",{})
    print("|"+"LIVE MARKET DISCOVERY".center(80)+"|")
    print("|"+f"FX {inventory.get('FX',0) if inventory else 0:>4} | Live {lb.get('FX',0):>3}    CRYPTO {inventory.get('CRYPTO',0) if inventory else 0:>4} | Live {lb.get('CRYPTO',0):>3}".ljust(80)+"|")
    print("|"+f"METALS {inventory.get('METALS',0) if inventory else 0:>3} | Live {lb.get('METALS',0):>3}  INDEX {inventory.get('INDEX',0) if inventory else 0:>4} | Live {lb.get('INDEX',0):>3}".ljust(80)+"|")
    if reasons:
        top=" | ".join(f"{k}:{v}" for k,v in sorted(reasons.items(),key=lambda x:-x[1])[:5])
        print("|"+f"Scanned: {scanned} | Live quotes: {d.get('live_total',0)} | Rejections: {top}".ljust(80)[:80]+"|")
    if samples:
...
def disconnected_dashboard(last_error, last_account=None, reconnect_in=RECONNECT_SECONDS):
    """Render a non-fatal paused state while MT5 is closed/unavailable."""
    os.system("cls")
    print("+"+"-"*80+"+")
    print("|"+"DEXORZO  V8.10  |  MT5 CONNECTION WATCHDOG".center(80)+"|")
    print("|"+BRAND.center(80)+"|")
    print("+"+"-"*80+"+")
    if last_account is not None:
        print("|"+f"Last Server: {getattr(last_account,'server','Unknown')}".ljust(40)+f"Balance: ${float(getattr(last_account,'balance',0)):,.2f}".ljust(40)+"|")
        print("|"+f"Last Equity: ${float(getattr(last_account,'equity',0)):,.2f}".ljust(40)+"Trading: PAUSED".ljust(40)+"|")
    else:
        print("|"+"Server: MT5 NOT CONNECTED".ljust(40)+"Balance: --".ljust(40)+"|")
        print("|"+"Equity: --".ljust(40)+"Trading: PAUSED".ljust(40)+"|")
    print("|"+"Selected Markets: 0/12".ljust(40)+"Open Entries: BLOCKED".ljust(40)+"|")
    print("+"+"-"*80+"+")
    print("|"+"!!! WARNING: METATRADER 5 IS OFFLINE / UNAVAILABLE !!!".center(80)+"|")
    print("|"+"The bot has NOT crashed. Trading is PAUSED safely.".center(80)+"|")
    print("|"+"Please switch/open MetaTrader 5 and reconnect to MetaQuotes-Demo.".center(80)+"|")
    print("|"+"The bot will automatically return to LIVE when MT5 is detected.".center(80)+"|")
    print("|"+f"Reason: {last_error}"[:80].ljust(80)+"|")
    print("|"+f"Next connection check: ~{reconnect_in}s".center(80)+"|")
    print("+"+"-"*80+"+")
    print("|"+f"Heartbeat: {datetime.now().strftime('%Y-%m-%d %H:%M:%S')} | Mode: PAUSED / MT5 WATCHDOG".ljust(80)+"|")
    print("|"+"Controls: STOP=halt bot | PAUSE=resume operator pause".ljust(80)+"|")
    print("+"+"-"*80+"+")

def main():
    print(BRAND)
    last_account=None; last="None"; error=""; states={}; seen={}
    ...
    try:
        while True:
            ...
            if ctl=="STOP":
                if last_account is not None:
                    try:
                        for p in positions(): close_position(p,"EMERGENCY_STOP")
                    except Exception:
                        pass
                print("\nV8.10 STOP active - bot halted.")
                break

            a, conn_error = connect()
            if a is None:
                disconnected_dashboard(conn_error or "MT5 unavailable", last_account)
                time.sleep(RECONNECT_SECONDS)
                continue

            a, guard_status = guard()
            if a is None:
                if guard_status.startswith("SAFETY LOCK"):
                    os.system("cls")
                    print("\n"+guard_status)
                    print("Bot halted for safety. Start it only on MetaQuotes-Demo.")
                    break
                disconnected_dashboard(guard_status, last_account)
                time.sleep(RECONNECT_SECONDS)
                continue

            last_account=a
            if not connected_notice:
                print("\nMT5 LIVE: connection restored. Bot resumed automatically.")
            ...
            try:
                ...
                if ctl=="STOP":
                    ...
                    dashboard(a,[],states,last,"STOP_V8_10 active - halted",{},0,{},[],{})
                    break
                manage_positions()
                selected,discovered,reasons,inventory,samples,diagnostics=select_pairs()
                ...
                test_a, test_t = mt5.account_info(), mt5.terminal_info()
                if test_a is None or test_t is None:
                    connected_notice=False
                    try: mt5.shutdown()
                    except Exception: pass
                    disconnected_dashboard(f"MT5 connection lost during cycle: {e}", last_account)
                    time.sleep(RECONNECT_SECONDS)
                else:
                    error=f"Runtime warning: {e}"
                    try:
                        dashboard(a,[],states,last,error,{},0,{},[],{})
                    except Exception:
                        pass
                    time.sleep(RECONNECT_SECONDS)
    except KeyboardInterrupt:
        print("\nV8.10 stopped by user.")
    finally:
        try: mt5.shutdown()
        except Exception: pass
