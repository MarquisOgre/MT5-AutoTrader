...
"""
DEXORZO Innovations
MT5 AutoTrader V8.7 - Dynamic M5
...
APP_TITLE = "MT5 AUTOTRADER V8.7 - DYNAMIC M5"
...
TRADES_FILE=REPORT/"v8_7_dynamic_m5_trades.csv"
STATE_FILE=RUNTIME/"v8_7_day_state.json"
STOP_FILE=Path("STOP_V8_7")
PAUSE_FILE=Path("PAUSE_V8_7")
RESUME_FILE=Path("RESUME_V8_7")
# Runtime state for partial-take-profit management.
PARTIAL_STATE_FILE=RUNTIME/"v8_7_partial_state.json"
...
def guard():
    ...
    if "MetaQuotes-Demo" not in str(a.server or ""):
        raise RuntimeError(f"SAFETY LOCK: V8.7 is restricted to MetaQuotes-Demo. Current server: {a.server}")
...
def session_gate(symbol, cls):
    # FX is explicitly skipped on weekends. This is intentional: it prevents the
    # dynamic scanner from spending its entire probe budget on closed FX symbols.
    if is_weekend_utc() and cls=="FX":
        return False,"FX_WEEKEND_CLOSED"
    sess=python_session_open(symbol)
    if sess is False:
        # Do not reject Crypto/other 24x7 assets merely because Python session
        # metadata is unavailable/incomplete; live quote remains the final gate.
        if cls=="FX": return False,"SESSION_CLOSED"
...
def dashboard(a,selected,states,last,error,reasons=None,scanned=0,inventory=None,samples=None):
    ...
    os.system("cls")
    print("+"+"-"*80+"+")
    print("|"+"DEXORZO  V8.7  |  INTELLIGENT 24x7 DYNAMIC M5".center(80)+"|")
    ...
    try:
        while True:
            ...
            if ctl=="STOP":
                ...
                dashboard(a,[],states,last,"STOP_V8_7 active - halted",{},0,{},[])
                break
            manage_positions()
