...
"""
DEXORZO Innovations
MT5 AutoTrader V8 - DEMO ONLY

V8 UI release retains risk/lifecycle controls on top of the V7.1 execution engine.
...
APP_TITLE = "MT5 AUTOTRADER V8 - DEMO ONLY"
...
TRADES_FILE = REPORT / "v8_demo_trades.csv"
DAY_STATE_FILE = RUNTIME / "day_start_equity.json"
STOP_FILE = Path("STOP_V8")
...
def demo_only_guard():
    ...
    if "MetaQuotes-Demo" not in server:
        raise RuntimeError(
            f"SAFETY LOCK: V8 only permits MetaQuotes-Demo. Current server: {server}"
        )
...
def send_entry(signal, account, positions):
    ...
    if emergency_stop_active():
        return False, "EMERGENCY STOP ACTIVE (STOP_V8 exists)"
...
def dashboard(account, states, last, error, session_dd):
    ...
    print(separator())
    print("Ctrl+C to stop. Create STOP_V8 in this folder to block NEW entries.")



# ---------------- V8 TRADE MANAGEMENT ----------------
MGMT=CONFIG.get("management",{})
PARTIAL_STATE_FILE=RUNTIME/"v8_partial_state.json"

def _pos_age_seconds(p):
    try:
        t=getattr(p,"time_msc",0)
        return max(0.0,time.time()-float(t)/1000.0) if t else max(0.0,time.time()-float(p.time))
    except Exception:
        return 0.0

def _r_multiple_live(p):
    if not p.sl or not p.price_open: return None
    risk=abs(float(p.price_open)-float(p.sl))
    if risk<=0: return None
    tick=mt5.symbol_info_tick(p.symbol)
    if not tick: return None
    cur=tick.bid if p.type==mt5.POSITION_TYPE_BUY else tick.ask
    reward=(cur-p.price_open) if p.type==mt5.POSITION_TYPE_BUY else (p.price_open-cur)
    return reward/risk

def _modify_sl_tp(p,new_sl=None,new_tp=None):
    req={"action":mt5.TRADE_ACTION_SLTP,"symbol":p.symbol,"position":p.ticket,
         "sl":norm_price(p.symbol,new_sl if new_sl is not None else p.sl),
         "tp":norm_price(p.symbol,new_tp if new_tp is not None else p.tp),
         "magic":MAGIC,"comment":"DEXORZO-V8-MGMT"}
    r=mt5.order_send(req)
    return r is not None and getattr(r,"retcode",None)==mt5.TRADE_RETCODE_DONE

def manage_positions_v8():
    ps=get_our_positions()
    partial={}
    if PARTIAL_STATE_FILE.exists():
        try: partial=json.loads(PARTIAL_STATE_FILE.read_text(encoding="utf-8"))
        except Exception: partial={}
    active={str(p.ticket) for p in ps}
    partial={k:v for k,v in partial.items() if k in active}
    for p in ps:
        tick=mt5.symbol_info_tick(p.symbol); info=mt5.symbol_info(p.symbol)
        if not tick or not info: continue
        cur=tick.bid if p.type==mt5.POSITION_TYPE_BUY else tick.ask
        r=_r_multiple_live(p)
        if MGMT.get("break_even",{}).get("enabled") and r is not None and r>=float(MGMT["break_even"]["trigger_r"]):
            off=float(MGMT["break_even"].get("offset_points",2))*info.point
            target=p.price_open+off if p.type==mt5.POSITION_TYPE_BUY else p.price_open-off
            better=(not p.sl) or (target>p.sl if p.type==mt5.POSITION_TYPE_BUY else target<p.sl)
            if better: _modify_sl_tp(p,target,p.tp)
        if MGMT.get("atr_trailing",{}).get("enabled") and r is not None and r>=float(MGMT["atr_trailing"]["trigger_r"]):
            raw=mt5.copy_rates_from_pos(p.symbol,mt5.TIMEFRAME_M15,0,80)
            if raw is not None and len(raw)>=30:
                df=enrich(pd.DataFrame(raw))
                a=float(df.iloc[-2]["atr"])
                dist=float(MGMT["atr_trailing"]["atr_multiplier"])*a
                target=cur-dist if p.type==mt5.POSITION_TYPE_BUY else cur+dist
                better=(not p.sl) or (target>p.sl if p.type==mt5.POSITION_TYPE_BUY else target<p.sl)
                if better: _modify_sl_tp(p,target,p.tp)
        if MGMT.get("partial_take_profit",{}).get("enabled") and r is not None and r>=float(MGMT["partial_take_profit"]["trigger_r"]) and not partial.get(str(p.ticket)):
            frac=min(.99,max(.01,float(MGMT["partial_take_profit"].get("close_fraction",.5))))
            step=float(info.volume_step or .01)
            v=math.floor((p.volume*frac)/step+1e-12)*step
            v=round(v,max(0,len(str(step).split(".")[-1].rstrip("0"))))
            if v>=float(info.volume_min or step) and v<p.volume:
                typ=mt5.ORDER_TYPE_SELL if p.type==mt5.POSITION_TYPE_BUY else mt5.ORDER_TYPE_BUY
                price=tick.bid if p.type==mt5.POSITION_TYPE_BUY else tick.ask
                req={"action":mt5.TRADE_ACTION_DEAL,"symbol":p.symbol,"volume":v,"type":typ,"position":p.ticket,
                     "price":price,"deviation":DEVIATION,"magic":MAGIC,"comment":"DEXORZO-V8-PARTIAL",
                     "type_time":mt5.ORDER_TIME_GTC}
                rr,msg=send_checked_order(req)
                if rr is not None: partial[str(p.ticket)]=True
        max_min=float(MGMT.get("max_time_in_trade_minutes",240))
        if max_min>0 and _pos_age_seconds(p)>=max_min*60:
            close_position(p,"MAX_TIME_IN_TRADE")
    PARTIAL_STATE_FILE.write_text(json.dumps(partial,indent=2),encoding="utf-8")

def main():
    print(DEXORZO_BANNER)
    print(f"{BRAND}")
    print("Starting MT5 AutoTrader V8 - Demo only\n")
    connect()
    account = demo_only_guard()
    get_day_start_equity(account)
    current_session_drawdown(float(account.equity))
    print(f"V8 safety lock passed: {account.server}")
